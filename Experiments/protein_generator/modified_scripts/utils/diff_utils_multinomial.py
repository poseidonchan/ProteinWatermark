...
def mask_inputs(seq, msa_masked, msa_full, xyz_t, t1d, input_seq_mask=None, 
    ...
    # print("seq_diffused_in_utils",seq_diffused)
...
def take_step_nostate(model, msa, msa_extra, seq, t1d, t2d, idx_pdb, N_cycle, xyz_prev, alpha, xyz_t,
    ...
    # print("Seq diffused", seq_diffused, seq_diffused.shape)

    seq_check = torch.argmax(seq_diffused,axis=-1)
    seq_check = seq_check.repeat(seq_check.shape[0], 1)
    # print("Seq check", seq_check)
    ...
    with torch.no_grad():
        ...
        # print("logit_aa_s", logit_aa_s.shape, logit_aa_s)
        
        ### CALCULATE ENTROPY FIRST ################################################
        ############################################################################
        # use (1 - qmax + qi)+ to update the logits
        
        true_logits = torch.clamp(2 - torch.max(logit_aa_s, 1, keepdims=True)[0] + logit_aa_s, min=0)
        probs = nn.functional.normalize(true_logits, p=1, dim=1) # (N, 21, L)
        entropy = -torch.sum(torch.log(probs+1e-4)*(probs+1e-4), dim=1) # (N, 21, L)
        
        print("Multinomial Entropy:", entropy.mean().item())
        
        ### ADD WATERMARK###########################################################
        seq_out = torch.zeros(B, L, dtype=torch.int64).to(logit_aa_s.device)
        # treat it as one-by-one generation process
        for i in range(logit_aa_s.shape[-1]):
            current_prob = probs[:,:,i]
            token_t = torch.multinomial(current_prob, 1)
            seq_out[:, i] = token_t
        ############################################################################
