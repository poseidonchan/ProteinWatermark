...
def take_step_nostate(model, msa, msa_extra, seq, t1d, t2d, idx_pdb, N_cycle, xyz_prev, alpha, xyz_t,
    ...
    with torch.no_grad():
        ...
        print("WaterMarked Entropy:", entropy.mean().item())
        
        ### ADD WATERMARK ##########################################################
        seq_out = torch.zeros(B, L, dtype=torch.int64, device=logit_aa_s.device)
        # treat it as one-by-one generation process
        for i in range(true_logits.shape[-1]):
            current_logit = torch.log(probs[:,:,i].reshape(B, -1))
            ...
            current_prob = torch.softmax(current_logit, dim=-1)
