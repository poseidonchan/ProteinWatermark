...
class SEQDIFF_sampler:
    ...
    def setup(self, init_model=True):
        ...
        self.features['seq_diffused'] = diff_utils_multinomial.mask_inputs(self.features['seq_hot'],
        ...
        # print("Init seq", self.features['seq'])
        # print("Init msa_masked", self.features['msa_masked'])
        # print("Init seq diffused", self.features['seq_diffused'])
        ...
        # get alphas
        self.features['alpha'], self.features['alpha_t'] = diff_utils_multinomial.get_alphas(self.features['t1d'], self.features['xyz_t'], 
    ...
    def predict_x(self):
        ...
        = diff_utils_multinomial.take_step_nostate(self.model,
    ...
    def generate_sample(self):
        ...
        for j in range(self.max_t):
            ...
            sampled_seq =  torch.clone(self.features['seq'][0])
            # get seq out
            self.features['seq_out'] = torch.permute(self.features['logit_aa_s'][0], (1,0))
            
            # save best seq
            if self.features['pred_lddt'].mean().item() > self.features['best_plddt']:
                ### Substitute it as watermarked seq
                # self.features['best_seq'] = torch.argmax(torch.clone(self.features['seq_out']), dim=-1)
                self.features['best_seq'] = sampled_seq
                self.features['best_pred_lddt'] = torch.clone(self.features['pred_lddt'])
                self.features['best_xyz'] = torch.clone(self.features['xyz'])
            ...
            print(''.join([self.conversion[i] for i in sampled_seq]))
...
class cleavage_foldswitch_SAMPLER(SEQDIFF_sampler):
    ...
    def setup(self, init_model=True):
        ...
        self.features['alpha'], self.features['alpha_t'] = diff_utils_multinomial.get_alphas(self.features['t1d'], self.features['xyz_t'],
                                                                                 self.features['B'], self.features['L'],
                                                                                 self.ti_dev, self.ti_flip, self.ang_ref)
        self.features['a_alpha'], self.features['a_alpha_t'] = diff_utils_multinomial.get_alphas(self.features['a_t1d'], self.features['a_xyz_t'],
                                                                                 self.features['a_B'], self.features['a_L'],
                                                                                 self.ti_dev, self.ti_flip, self.ang_ref)
        self.features['b_alpha'], self.features['b_alpha_t'] = diff_utils_multinomial.get_alphas(self.features['b_t1d'], self.features['b_xyz_t'],
...
class grad_test_SAMPLER(SEQDIFF_sampler):
    ...
    def predict_x(self):
        ...
        self.features['pair_prev'], \
        self.features['state_prev'] \
        = diff_utils_multinomial.take_step_nostate_grads(self.model,
        self.features['msa_masked'],
        self.features['msa_full'],
        self.features['seq'],
        self.features['t1d'],
