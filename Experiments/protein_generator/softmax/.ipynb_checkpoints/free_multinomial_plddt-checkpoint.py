...
filename = './free_multinomial_softmax.csv'
...
def average_pLDDT(seq):
    ...
    with open("free_multinomial_result.pdb", "w") as f:
        f.write(output)
    struct = bsio.load_structure("free_multinomial_result.pdb", extra_fields=["b_factor"])
